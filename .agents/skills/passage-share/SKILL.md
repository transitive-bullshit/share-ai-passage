...
Authenticated preparation requires `--out` and writes a recovery file before dispatch. If interrupted, use `resume <draft-file>` to continue the saved preparation or `status <draft-file>` to retrieve its current state. These commands preserve request identity and can return an already-published passage. Customize uploaded backgrounds, logos and reusable templates in the webapp; paid creation uses the saved default template. Review generation limits or failed preparation before publishing, and recover the existing request instead of starting another generation automatically.
