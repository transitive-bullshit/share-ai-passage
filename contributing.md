...
- The reader preserves extracted text and Markdown, folds reasoning/activity by default, highlights and copies fenced code, and supports wide desktop tables with contained mobile scrolling. Links use local favicon glyphs. Unsupported media, tools, and artifacts retain explicit omission markers. Provider HTML is not executed and remote media is not loaded.
