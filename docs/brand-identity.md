...
The product currently accepts public ChatGPT, Codex, Claude, and Gemini conversation links, generates a title and optional highlights that the sharer can refine before publication, offers five curated card styles, and publishes a passage with saved conversation text and its original source. Published wording stays fixed. Browser, CLI, and agent workflows serve this same purpose. Current scope and limits live in the [MVP plan](MVP_PLAN.md); the brand does not add accounts, collections, or custom themes.
...
Header and footer creation CTAs link to `/create` and use rounded shadcn outline buttons with a trailing Lucide external-link icon. The footer’s Source code link uses text only, retaining its accessible new-tab label. The header uses the default size; the footer uses the more compact small size.
...
The creation page lives at `/create?passage=<id>` and follows the passage’s saved state. Unpublished work uses “Review your passage” without a period; published work shows “Your passage is published” and its saved link, including after Back or refresh. Style choices show the passage’s current edited title and highlights in visual thumbnails with accessible names, without visible theme names or preview captions. Typing preserves the fitted canvas and artwork in both thumbnails and the main preview; only genuine wrapping or fit changes should move the text. Leave 24 px above the paid “Customize this passage” disclosure. Uploaded artwork uses “Background position,” “Horizontal position,” and “Vertical position”; these controls position the image. Title guidance is roughly 10 words at most (usually 4–7), leading with the most distinctive terms; highlights recommend 100 characters. All title/highlight word and character guidance is advisory; text length never blocks editing or publication. Sharers can add or remove up to three highlights; blank highlights and empty highlights sections are omitted. Cards fit text to their layout, clipping titles to two lines and each highlight to three lines with an ellipsis. Keep full text in saved passages and readers. Use the best fitting card display without a readability rejection.

Share card titles use balanced text wrapping and occupy at most two lines, with an ellipsis when the title overflows. Use a minimum line height of 1.2 so clipping keeps glyph ascenders and descenders intact, including in older saved custom designs. Keep the full title in the saved summary and reader. Card footers show the source attribution or example mantra. Reserve **Read the passage** for the interactive link that opens a published passage; omit it from card templates and their exported images.
...
The build uses tsx and the development-only Resvg and `@shuding/opentype.js` dependencies. Application cards use the separate Takumi renderer. It checks the JSON render projection and shared site copy against this guide. SVG one-pager/social outputs retain editable text with embedded font and image inputs; PNG output outlines the same fonts. The command also updates the application favicon files and `public/brand/social-preview.jpg` (progressive JPEG at quality 85, encoded from the PNG master with Sharp).

The README shows the [before-and-after comparison](../public/images/passage-before-after.jpg), linked to the production homepage. [marketing-examples.ts](../lib/marketing-examples.ts) records the featured conversation's public source and both production database publication URLs. The homepage and README example text link directly to the featured reader; legacy example URLs redirect to the corresponding readers. Local image routes and retained WebP exports use the reviewed wording, matching styles, and provider attribution. These publications depend on the original source remaining public. Place links to try Passage, read an example, and run locally before the README image. Link the site’s agent section to its setup instructions and skill. See [maintenance instructions](../contributing.md#marketing-examples).

**Export verification (September 12, 2026):** Actual PNG dimensions and all three embedded ICO entries were checked. Application icons and the original PNG social artwork were byte-identical to their canonical masters at verification time; the deployed social preview now uses an optimized JPEG. SVGs have no external asset references; standalone copies reproduced identical PNGs using only embedded fonts and artwork with system fonts disabled. All PNG compositions and the marks at 16–96 px were visually inspected. The live homepage was checked at desktop and 375 px, and the new favicon, social image, metadata, and example-card routes were verified. After integrating the Takumi renderer, `pnpm test` passed all 407 tests with a migrated disposable local PostgreSQL database, including 37 card/image-route tests, and `pnpm build` passed. Both example readers and their 1200 × 630 WebP routes returned 200 from the local production build; unknown examples returned 404. The exported images and the same templates rendered as HTML in Chrome were visually checked for font, artwork, and text fit.
