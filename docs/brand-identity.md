...
The README shows the same real conversation published in Margin notes and Midnight observatory. [marketing-examples.ts](../lib/marketing-examples.ts) records its public source and both production database publication URLs. The homepage and README link directly to those readers; legacy example URLs redirect there. Local image routes and README WebP exports use the reviewed wording, matching styles, and provider attribution. These publications depend on the original source remaining public. Place links to try Passage, read an example, and run locally before the README images. Link the site’s agent section to its setup instructions and skill. See [maintenance instructions](../contributing.md#marketing-examples).
