...
`smoke` checks preparation, rejected invalid preview edits, idempotent publication, seven-day reader and 30-day card cache headers, reader metadata, the HTML card preview with bundled assets, and matching draft/public WebP images, including their content type and dimensions. It creates publications and consumes two preparation attempts per source. Uncached sources can incur model charges; saved previews are reused. `smoke:removal` creates and cleans up synthetic records, warms two publication caches, leaves one cold, disables their shared source, and verifies both intended outcomes: fresh ISR entries remain available until revalidation, while the cold reader, RSC, metadata, and card render the unavailable state. It also checks immediate draft rejection. Provider-check behavior is covered separately by lifecycle tests. The creation page uses an HTML preview; check its font and artwork loading, text edits, text fit, template changes, and publish readiness in a browser alongside the WebP output when changing draft review or card rendering.
