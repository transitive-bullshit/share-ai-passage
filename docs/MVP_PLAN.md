...
The [accounts and paid features handoff](ACCOUNTS_PAID_FEATURES_PLAN.md) defines the expansion. Accounts passed the owner’s Gate A review on September 15, 2026 (`15755bc`). Phase 2 billing, customization, assets, durable image jobs and authenticated CLI are committed locally in `e5f43dd`; see the [paid review](PAID_REVIEW.md) for validation. Stripe sandbox and development R2 setup are verified. Gate B remains pending the remaining billing lifecycle checks, production R2 delivery, hosted Workflow verification, and complete model quality/cost qualification. Neither phase has been deployed or migrated to production. See the [accounts review](ACCOUNTS_REVIEW.md), [measured economics](research/PHASE2_MEASURED_ECONOMICS.md), and launch checks below.
...
The original MVP deployment was recorded on September 11, 2026, with Neon configured and Vercel deployed. The accounts/paid expansion has not been deployed and Gate B remains open. Recorded sandbox billing and R2 checks are in the [paid review](PAID_REVIEW.md). A new synthetic failed-renewal check used real app wall time: expired coverage blocked paid actions, and paying the same invoice restored Plus's 100-summary/10-image allowances without duplicate grants. Its exact fixtures were removed and independently verified; it is direct API/application evidence, not hosted or browser recovery proof.
...
Remaining gates include unseen out-of-order billing events, disputes, failed annual payment/recovery, interrupted account deletion, production R2 delivery, hosted webhook/Workflow lifecycle checks, and qualified image quality/input costs. The remaining 25-call qualification batch awaits the explicit approval requested after automatic approval review rejected it; no batch call ran. The [measured report](research/PHASE2_MEASURED_ECONOMICS.md) separates existing observations from unverified margins.
