...
Previously unseen old paid/active events also preserved cancellation and Free access after newer cancellation reconciliation; their exact synthetic fixtures were removed. Injected-timeout account deletion recovery also passed through the actual cancellation CLI, including a late subscription after modeled profile removal. The corrected annual transition also withheld unpaid access after a real $96 decline and restored Plus only after that same invoice was paid; grant replay and exact cleanup passed. Remaining gates include disputes, production R2 delivery, hosted webhook/Workflow lifecycle checks, and qualified image quality/input costs. The remaining 25-call qualification batch awaits the explicit approval requested after automatic approval review rejected it; no batch call ran. The [measured report](research/PHASE2_MEASURED_ECONOMICS.md) separates existing observations from unverified margins.
