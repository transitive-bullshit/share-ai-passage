...
const data: Data = JSON.parse(
  await readFile(join(here, 'brand-data.json'), 'utf8')
)
...
for (const d of data.directions) {
  for (const [key, weight] of [
    ...
    const bytes = await readFile(
      join(here, 'fonts', d.fontFamily + '-latin-' + weight + '-normal.woff')
    )
    fonts[key] = {
      bytes,
...
for (const d of data.directions) {
  const spec = normalize(
    await readFile(join(here, '../brand-identity.md'), 'utf8')
  )
  for (const value of [
    d.name,
    d.territory,
...
assert.equal(direction.name, brand.name)
assert.equal(direction.headline.join(' '), brand.headline)
assert.equal(direction.descriptor, brand.description)
assert.equal(direction.cta, brand.cta)
assert.equal(direction.mantra, brand.mantra)
const markSvg = (color: string) =>
  '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32" role="img"><title>Passage</title>' +
  brand.markPaths
    .map((path) => '<path d="' + path + '" fill="' + color + '"/>')
    .join('') +
  '</svg>'
const source = markSvg('currentColor')
await writeFile(join(here, 'logo-mark.svg'), markSvg('#171717'))
await writeFile(join(here, 'logo-mark-inverse.svg'), markSvg('#FFFFFF'))
...
function poster(card: Canvas) {
  ...
  c.text('Passage · Brand identity', 80, 1260, 18, 'passage', '#737373')
  c.text(
    'Accepted September 12, 2026 · illustrative passage',
    1520,
    1260,
    18,
    'passage',
...
function socialPreview() {
  const c = new Canvas(1200, 630, brand.name + ' — ' + brand.headline)
  c.rect(0, 0, 1200, 630, '#FFFFFF')
  c.photo(direction, 800, 0, 400, 630)
  wordmark(c, 64, 107, 46)
  direction.headline.forEach((line, i) =>
    c.text(
      line,
      64,
      241 + i * 78,
  ...
  c.wrap(brand.description, 64, 382, 640, 25, 36, 'passage', '#737373')
  c.rect(64, 466, 254, 60, '#171717', 30)
  c.text(brand.cta, 191, 504, 22, 'passageMedium', '#FFFFFF', {
    align: 'center',
    maxWidth: 218
  })
  c.text(brand.mantra, 64, 582, 18, 'passage', '#737373', { maxWidth: 670 })
  return c
}
for (const [label, color] of [
  ['logo-wordmark', '#171717'],
  ['logo-wordmark-inverse', '#FFFFFF']
] as const) {
  const c = new Canvas(280, 80, 'Passage wordmark')
  wordmark(c, 12, 58, 56, color)
  await writeFile(join(here, label + '.svg'), c.svg(true))
}
const favicon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32" role="img"><title>Passage</title><rect width="32" height="32" rx="8" fill="#171717"/>' +
  brand.markPaths
    .map((path) => '<path d="' + path + '" fill="#FFFFFF"/>')
    .join('') +
  '</svg>'
await writeFile(join(here, 'favicon.svg'), favicon)
const sizes = [16, 32, 48]
const iconImages = sizes.map((size) =>
...
sizes.forEach((size, i) => {
  ...
  header.writeUInt32LE(iconImages[i]!.length, start + 8)
  header.writeUInt32LE(offset, start + 12)
  offset += iconImages[i]!.length
})
const ico = Buffer.concat([header, ...iconImages])
await writeFile(join(here, 'favicon.ico'), ico)
const card = passageCard()
const sheet = poster(card)
const social = socialPreview()
for (const [label, canvas] of [
  ['brand-identity-onepager', sheet],
  ['example-passage', card],
  ['social-preview', social]
] as const) {
  await writeFile(join(here, label + '.svg'), canvas.svg())
  await writeFile(
    join(here, label + '.png'),
    new Resvg(canvas.svg(true)).render().asPng()
  )
}
await mkdir(join(here, '../../public/brand'), { recursive: true })
  ...
  JSON.stringify(
    {
      ...
      accepted: '2026-09-12',
      onepager: [1600, 1300],
      examplePassage: [1200, 630],
      socialPreview: [1200, 630],
      faviconSizes: sizes,
      measuredTextRuns: measurementRecords.length,
      iconSource: 'lib/brand.ts',
      copy: {
        headline: brand.headline,
        descriptor: brand.description,
        cta: brand.cta,
...
console.log(
  'Built the accepted Passage identity, app favicons, and social preview'
)
