...
A read-only Vercel billing check at **10:12 UTC on September 15** returned 48 charge rows tagged to Passage on the existing Pro plan. The requested period was September 15–16 UTC; the latest returned charge period ended at **07:00 UTC**, so current-day coverage was incomplete. Selected API-reported aggregates follow. These are project-wide billing quantities across deployment environments, not raw request counts, Preview/per-user costs or a final invoice. Workflow and queue rows reported zero within that lagged export, which does not establish zero Preview usage. A separate Preview-only function query returned HTTP 402 requiring Observability Plus; no purchase was made or added as a launch prerequisite. [Aggregate evidence](../../work/phase2/hosted-readiness/vercel-usage-evidence.json), [billing contract](https://vercel.com/docs/rest-api/billing/list-focus-billing-charges), [metrics access](https://vercel.com/docs/cli/metrics).

| Reported service | Billing quantity | Reported billed cost |
| --- | --: | --: |
| Function Invocations | 1,036.095238 invocation units | $0.000622 |
| Fluid Active CPU | 0.029135 hours | $0.003729 |
| Fluid Provisioned Memory | 0.160134 GB-hours | $0.001697 |
...
The current GPT Image 2.5 guide calls its output calculator an estimate and directs applications to measure returned usage. The vision guide explicitly excludes GPT Image from its patch/tile calculator; its older GPT Image 1 rules cannot establish Flare 2.5 input costs. The Images API exposes no `max_output_tokens` or per-request spending cap. A documented hard per-call ceiling would require **provider-confirmed maximum billable text/reference/output tokens, including overhead, for this pinned configuration**. That assurance is unavailable; more ordinary samples or a larger compressed reference cannot prove it. After the owner accepted sampled economics with per-account spending protection, provider confirmation or another benchmark is not a remaining launch prerequisite. [Generation costs](https://developers.openai.com/api/docs/guides/image-generation), [GPT Image input accounting](https://developers.openai.com/api/docs/guides/images-vision#gpt-image-model-inputs), [Images API](https://developers.openai.com/api/reference/resources/images/methods/generate).
