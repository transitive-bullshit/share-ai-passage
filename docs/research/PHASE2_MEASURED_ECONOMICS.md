...
The independent reviewer inspected 32 full-resolution production cards across all five layouts: 30 ordinary cards were readable. The two maximum-prompt probe cards had valid but extreme 1000/1000/637-character highlights; the existing text fitter made them illegible. **Full supported-input readability remains conditional on a renderer/input-policy resolution.** This is an offline rendering issue, not evidence that more image calls would fix it. [Qualification report](../../work/phase2/image-benchmark/QUALIFICATION_REPORT.md), [gallery](../../work/phase2/image-benchmark/qualification.html), [metrics](../../work/phase2/image-benchmark/qualification-metrics.json).
...
The decision-ready alternative is to explicitly accept sampled economics with **account-level AI spend admission and an anomaly pause**, rather than wait for a hard-cap formula that is not published. Under the existing fee/reserve assumptions, the 70% contribution target leaves a combined summary+image budget of **$1.587 per Plus annual month** and **$4.255 per Pro annual month**; the corresponding monthly-plan budgets are $1.840/$5.300. A fully redeemed $10 pack leaves **$2.210 for model spend across 50 credits** after $0.59 fees and $0.20 other reserve. These are candidate operating ceilings derived from the accepted economics, not new generation allowances or proven margins.

A minimal proposed admission check would sum actual provider costs plus unresolved reservations for the original account/grant period, include the next reservation, and pause that account/configuration when it exceeds the chosen ceiling or metering is unresolved. Operator review would reconcile costs before resuming. The current implementation has the global monthly budget but **does not implement these per-account/grant ceilings or a unit-cost anomaly pause**. Adding them requires an explicit decision about temporary unavailability while nominal credits remain; it must not silently shrink 10/25 or 50 credits. Non-model reserves still need operational validation, and any first provider charge could exceed its operational reservation. No implementation or additional paid benchmark is authorized by this report.
