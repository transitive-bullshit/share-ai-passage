...
Cleanup is still manual: automatic approval review rejected adding the proposed daily production job without explicit authorization, so its implementation is parked pending the owner's answer. Private-cleanup scheduling, production credential scope/delivery, hosted recovery and measured R2 usage remain Gate B checks.
