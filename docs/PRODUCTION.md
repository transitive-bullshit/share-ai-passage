...
Still pending: a durable live Stripe runtime key, Portal/webhook setup and their production variables. The old Agentic CLI live credential is expired. Automatic approval review rejected initiating a full-access live key without specific authorization; the owner was asked to approve a named `Passage Production` key saved only in private production configuration. Google console inspection also awaits the owner's passkey verification. Its last observed audience is External/Testing; identity-only `openid`, email and profile requests are [exempt from test-user restrictions, warnings and seven-day expiration](https://support.google.com/cloud/answer/15549945?hl=en), but that exception does not complete branding verification.

The accounts application has not been merged or deployed to production. After Gate B and merge, verify production sign-in, signup email, preparation Workflow and published reader/card access; configure/enable the live signed webhook and explicitly enable live Checkout only when its handler is deployed. Keep other Agentic products and account-wide branding/Portal settings intact. Native Vercel cron schedules activate on the production application deployment; the configured secret authorizes daily asset cleanup and hourly subscription-email retries.
...
The last verified publishing status, from September 14 setup, is Testing with no test users added; Publish app was disabled pending complete branding configuration. A September 15 console read confirmed the separate `Passage` project and `Passage production` Web application client, but Audience did not load. The September 17 production console check awaits the owner's passkey. Open the [production client list](https://console.cloud.google.com/auth/clients?authuser=1&project=share-ai-passage-production) in that account. The PR now implements `/privacy` and its homepage/footer link; deploy them before registering the public policy URL and completing branding/domain-ownership checks. Identity-only requests have the Testing exceptions documented above; production callback and branding verification are still unverified. The API Services User Data Policy agreement was already accepted with the owner's explicit approval.
...
**Live catalog prepared September 17:** Agentic account `acct_1Qz2IuFRpaq9IkEX` has the following saved live prices. Dashboard reads confirmed each price/interval and zero active subscriptions. Runtime access and signed webhook/Portal configuration remain pending; the four price-ID variables alone do not enable billing.
...
The existing default management Portal, `bpc_1Rf0mTFRpaq9IkEXDW292xIE`, was read without changes: invoice/payment management and period-end cancellation are enabled; plan/quantity switching are disabled. A separate nondefault `Passage checked upgrades` shell, `bpc_1UGirdFRpaq9IkEXnnUZ7jxV`, was created with saved headline `Confirm your Passage upgrade` and return URL `https://www.share-ai-passage.com/account/billing`. Its no-code login link remains inactive. The Dashboard disables its advanced subscription switches, so it is **not ready for upgrades**: after live-key approval, configure/verify the sandbox-equivalent price-only product list, immediate invoicing and unchanged billing anchor through the API, then save `STRIPE_UPGRADE_PORTAL_CONFIGURATION_ID`. Reload confirmed the original Default and saved separate shell. No new general-Portal environment variable is needed with the verified safe default. Evidence: `work/production-launch/portal-preparation.json`.
...
Keep Vercel authentication on Preview, sandbox Stripe, live checkout disabled and `SUMMARY_AI_MONTHLY_BUDGET_USD=1`. Preserve summary service/account spending protection and the operations digest/cleanup guidance in [reconciliation](GENERATION_RECONCILIATION.md). Production migrations and the initial $25/month service-wide summary admission ceiling are prepared; live Stripe configuration, Gate B and explicit application/Checkout activation remain.
