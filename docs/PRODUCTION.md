...
September 18 follow-up: the owner explicitly approved creating the full-access `Passage Production` live Stripe key and saving it only in private production configuration. Stripe now requires the owner's separate Touch ID/security-key identity check; key creation, Portal/webhook API completion and their production variables remain pending. The old Agentic CLI live credential is expired. Google console access is restored: saved the production `/privacy` policy URL, published the External audience to **In production**, and started its automated branding checks. Google confirms that data-access verification is unnecessary for the existing identity-only scopes; branding verification is still incomplete. The policy page and its homepage link must become publicly available with the accounts deployment before completing review.
