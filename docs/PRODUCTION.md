...
The stable origin is `https://share-ai-passage-git-codex-accounts-first-launch-saasify.vercel.app`. Vercel Standard Protection redirects anonymous requests, blocking public access and Stripe delivery. The latest GET and POST probes returned 302; an earlier deployment returned POST401. Automatic approval review rejected the proposed exact-domain exception; it was not applied and explicit user approval is pending. No bypass credential was created. Both exact OAuth callbacks are saved. Hosted Google linking, returning sign-in and session reload pass through the owner’s Vercel session. Fresh GitHub sign-out/sign-in at `e0182da` returned to the signed-in homepage; Chrome blocked the subsequent account-page check, so no new post-GitHub reload or password setup is claimed. See the [hosted account evidence](PAID_REVIEW.md#hosted-preview). Signed webhook delivery remains unverified.
...
The isolated HTTPS review hostname uses the local Stripe listener at backend 3107. The shared Preview also now has sandbox endpoint `we_1UFrmhFCzsOIkvWCJdcVsOEU` at its stable origin plus `/api/auth/stripe/webhook`, with its own signing secret saved and redeployed. API readback verified all 20 events below; signed delivery is still blocked by Vercel protection (currently a redirect). Evidence: `work/phase2/hosted-readiness/stripe-webhook-evidence.json`. The local listener at `work/phase2/stripe/listen.mjs` forwards the same event set:
...
Cleanup is still manual: automatic approval review rejected adding the proposed daily production job without explicit authorization, so its implementation is parked pending the owner's answer. Private-cleanup scheduling, production credential scope/delivery and measured R2 usage remain Gate B checks. Hosted safe recovery retries are recorded below.
