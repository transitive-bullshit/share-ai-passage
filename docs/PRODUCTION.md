...
The standard Stripe Portal handles payment details and hosted confirmation. Configure cancellation at period end; Passage owns scheduled downgrades and billing-cadence changes. Hosted immediate upgrades must apply proration and require payment. Passage grants paid access only from confirmed paid state, not the redirect or native subscription mirror. Verify these settings and actual amounts in the sandbox before enabling live prices: new subscription, renewal failure/recovery, annual monthly grants, immediate upgrade, scheduled downgrade/cadence change, cancellation/resubscription, duplicate/out-of-order webhooks, refunded/disputed packs and interrupted account deletion. Deletion commits a retained closing marker before cancellation; late subscriptions are canceled through reconciliation even after the auth profile disappears.

### R2

Use separate public-card and private-asset buckets per environment with bucket-scoped Object Read & Write credentials. Set `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, both bucket names and `R2_PUBLIC_URL`. The optional `R2_ENDPOINT` must be an HTTPS R2 endpoint for that account. Public delivery belongs only to composed immutable cards; uploaded backgrounds, logos, references and generated artwork remain private and are read with short signed URLs.

Configure private-bucket CORS for exact application origins, GET/HEAD/PUT, and the request headers actually signed by the upload flow (`Content-Type`, `Content-Length`, `If-None-Match`); expose ETag if needed by the client. Verify a real browser upload and its signed size/conditional-write behavior against R2. The application validates normalized content, rejects files above 10 MB and reserves the 1 GB active upload-library limit atomically; actual R2 ingress enforcement is not established by fixture tests. Use short staging expiry and a staging-prefix lifecycle rule as a backstop. Do not add lifecycle deletion to immutable public cards or private generated results awaiting reconciliation.

Account deletion queues private-asset cleanup; unfinished image results are protected until their outcome settles. Public assets intentionally remain. Passage reader/card routes still enforce source and publication availability; direct public object URLs are intentionally not revocable. Run `pnpm cleanup:assets --apply --limit 50` with explicit target `DATABASE_URL` and R2 variables; it loads no env files and reports only examined/cleaned/skipped/failed counts. Private-cleanup scheduling, real credential scope/CORS, object persistence/recovery and R2 usage remain Gate B checks.
...
**Gate B is open:** actual Stripe sandbox and R2 integration are unverified, the image model qualification and supported input-cost bounds are incomplete, and hosted operational measurements/alerts and recovery remain outstanding. The [measured economics report](research/PHASE2_MEASURED_ECONOMICS.md) distinguishes observed sample costs from workload assumptions. Keep public paid checkout and new image generation disabled until the complete paid flow, quality, economics and deployment checks pass.
