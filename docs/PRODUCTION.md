...
On September 15, a read-only Vercel dashboard check found the existing `DATABASE_URL`, `DIRECT_DATABASE_URL`, `APP_SECRET` and `OPENAI_API_KEY` records scoped jointly to Production and Preview. No accounts-branch preview is deployed. The schema-only Neon branch `accounts-first-launch-preview` (`br-wild-union-ayj1kszx`) was created without customer rows and expires September 22, 2026 at 07:38:08 UTC. Its dedicated role/database, migrations and Vercel overrides are not yet configured. Before pushing/deploying this branch to Preview, add branch-specific database/auth overrides pointing to that dedicated migrated preview database; a new preview currently inherits production settings. Keep all Production values unchanged.
