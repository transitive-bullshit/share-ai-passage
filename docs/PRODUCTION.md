...
The stable origin is `https://share-ai-passage-git-codex-accounts-first-launch-saasify.vercel.app`. Vercel Standard Protection currently redirects anonymous GET requests (302) and rejects POST requests (401), blocking account entry and Stripe delivery. Automatic approval review rejected the proposed exact-domain exception; it was not applied and explicit user approval is pending. No bypass credential was created. GitHub’s exact callback is saved; Google setup awaits the owner’s passkey. Configuration and READY status do not establish hosted sign-in or signed delivery.
...
The isolated HTTPS review hostname uses the local Stripe listener at backend 3107. The shared Preview also now has sandbox endpoint `we_1UFrmhFCzsOIkvWCJdcVsOEU` at its stable origin plus `/api/auth/stripe/webhook`, with its own signing secret saved and redeployed. API readback verified all 20 events below; signed delivery is still blocked by Vercel protection (401). Evidence: `work/phase2/hosted-readiness/stripe-webhook-evidence.json`. The local listener at `work/phase2/stripe/listen.mjs` forwards the same event set:
