...
The existing `APP_SECRET` and automatic platform origin serve Better Auth; no new auth-secret names or account credential framework were added. Shared Preview reuses development Google/GitHub/Resend, sandbox Stripe catalog/key and existing R2 values. `STRIPE_LIVE_CHECKOUT_ENABLED=false` and `IMAGE_GENERATION_ENABLED=0` remain set. Evidence: `work/phase2/hosted-readiness/shared-preview-env-evidence.json`. The endpoint-specific Stripe signing secret uses the normal `STRIPE_WEBHOOK_SECRET` name.
