...
Google console access is restored: saved the production `/privacy` policy URL and published the External audience to **In production**. Its automated branding checks identified domain ownership and policy-content issues. Added Google's exact root TXT to Vercel without editing existing DNS records; Search Console confirmed domain ownership for `travis@transitivebullsh.it`. Google confirms that data-access verification is unnecessary for the existing identity-only scopes. The post-deployment check passed on September 18 and the verified branding is now published, as recorded below.

The owner merged PR #4 and deployed `fedb92d` on September 18. The accounts application, privacy page, daily asset-cleanup cron and hourly subscription-email cron now serve production. The prepared live webhook is enabled; Stripe records a delivered `200 OK` for an existing `invoice.paid` event confirmed not to map to any Passage account. Invalid signatures return 400, valid signed requests and replay return 200, and Passage billing records remain unchanged. Checkout stayed disabled during this initial review and was enabled afterward with explicit owner authorization, as recorded below. No live subscription or payment was created. Keep other Agentic products and account-wide branding/Portal settings intact.

### Production smoke review — September 18

Normal signup to Resend's official delivery test sink, delivery of its real verification link, email verification and subsequent password sign-in passed. Guest creation persisted its passage ID, completed a real preparation Workflow, generated wording, regenerated once and published a readable conversation/card. Both successful generations settled at a combined $0.000498 with no outstanding budget reservation. Signing into the verified test account imported the guest passage and both quota charges: Free showed 2 used of 25. Valid Free upload/template requests returned `403 PAID_ACCOUNT_REQUIRED`; private account responses remain `private, no-store`, unauthenticated access is denied, and cron requests without their secret return 401. Existing public readers and social cards return 200 with Vercel cache hits; pricing and privacy remain prerendered. All 17 migration ledger entries are present, with no stale preparations or pending email outbox work at the initial review.

Google's automated production branding checks passed after the deployed privacy link became available. Published the verified branding; the console confirms it is shown to users. The owner's identity-only Google connection completed its production callback and Account showed both Google and GitHub connected. The existing GitHub session also persisted across navigation. This review did not repeat a fresh GitHub login or change the owner's password.

The review found a browser-history defect when creation starts directly on `/create`: after publishing, opening the reader and pressing Back restored a blank creation form despite retaining `?passage=<id>`. Native `replaceState` preserved Next's original server-page snapshot without its passage identity. The follow-up uses `router.replace`; 10 focused regressions, lint and TypeScript pass. On protected Preview at `4880b87`, the exact create → publish → reader → Back loop restores the published result; cold reload also does so, and Create another passage clears the identity. This fix is not yet deployed to production. The complete hosted paid upload → template → branded publication journey remains unverified. Normal deletion of the synthetic verified account returned 200, removed its profile/private drafts and disabled both test publications. The unused invalid-address fixture was also removed; public R2 assets and cost audit records were preserved. Owner-only receipts are under ignored `work/production-review/`.

### Live billing activation — September 18

With explicit owner authorization, set `STRIPE_LIVE_CHECKOUT_ENABLED=true` in Vercel Production and the main private `.env.prod.local`. Redeployed the current production source `fedb92d` as `dpl_HQdzedT2soVEfEuJYZtYmE37PSx5`, verified Ready with production aliases assigned, and confirmed Plus/Pro buttons are enabled. This configuration redeploy does not include the unmerged history/title follow-up fixes.

The production Plus button opened live Stripe Checkout for Passage Plus at $10/month. Stripe API readback confirmed the correct live price, quantity one, unpaid status and production return URLs; the session was then expired. No payment or subscription was created. Merchant charges/payouts, all four prices, both Portal policies and the exact live webhook contract passed read-only verification. After redeployment, valid signed requests for an unmapped customer returned 200 and invalid signatures returned 400; homepage, pricing and account configuration returned 200.
...
Separate `Passage development` and `Passage production` API keys have Sending access restricted to this domain. Development and shared Vercel Preview reuse the development key; Production retains its separate key. Values are saved privately in Vercel and the main checkout’s owner-only `.env.local` / `.env.prod.local`. Preview configuration and existing-account password email delivery are verified. September 18 normal production signup, delivered verification email, first-party verification and password sign-in passed using Resend's official delivery test sink.
...
The sender is a sending identity, not a receiving mailbox. The owner reports creating a Google Workspace group alias for Reply-To that forwards to `travis@transitivebullsh.it`; external delivery has not been tested. The accounts implementation consumes these variables for verification/password-reset emails. Development configuration is integrated in the isolated accounts review. On September 15, the owner completed a delivered password reset, and subsequent email/password sign-in returned to the same account with Google and GitHub still linked. At that revision, fresh signup verification delivery remained untested live; the September 18 production smoke review above now verifies it. Accounts are deployed to shared Preview. The owner also completed hosted password setup and signed in with the updated password; a subsequent browser reload showed all three methods connected. Fresh signup verification passed on production on September 18; external Reply-To delivery remains untested. Vercel environment changes apply to subsequent deployments.
...
The Web application client `Passage production` has JavaScript origin `https://www.share-ai-passage.com` and exact redirect URI `https://www.share-ai-passage.com/api/auth/callback/google`. Its client ID and secret are saved as `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` in the main checkout's ignored, owner-only `.env.prod.local`. Both were also saved and verified as Production-only Secret environment variables in [Vercel's saasify/share-ai-passage project](https://vercel.com/saasify/share-ai-passage/settings/environment-variables) on September 14, 2026 and are now deployed. Production credentials must not be reused for development or Preview; the separate development client is recorded below.

On September 18, after the owner completed passkey sign-in, the production console accepted and saved privacy URL `https://www.share-ai-passage.com/privacy`; the existing name, logo, support/contact, homepage and authorized domain were preserved. Published the External audience and verified **In production** on Audience. The exact Google ownership TXT was added to the root in Vercel, preserving existing DNS settings, and Search Console confirmed ownership of `share-ai-passage.com` for `travis@transitivebullsh.it`. Keep that verification TXT. After `/privacy` and its homepage/footer link deployed, branding verification passed and the verified branding was published. Verification Center confirms it is shown to users and no sensitive-scope verification is required. The owner's production callback/session passed, and deployed sign-in initialization uses the exact production client and redirect. Open the [production client list](https://console.cloud.google.com/auth/clients?authuser=1&project=share-ai-passage-production) in the organization account. The API Services User Data Policy agreement was already accepted with the owner's explicit approval.
...
The existing `APP_SECRET` and automatic platform origin serve Better Auth; no new auth-secret names or account credential framework were added. Shared Preview reuses development Google/GitHub/Resend, sandbox Stripe catalog/key and existing R2 values. Preview live Checkout remains disabled. The protected Preview retains a $1/month summary operational budget; image generation is removed.
...
Phase 2 is implemented and deployed to shared Preview and production. Preview, local review/test and production databases are through `0016_subscription_emails`, including durable preparation, uploads-only design normalization, removal of the title length ceiling and subscription emails. Production migration followed an independently verified backup/restore and rehearsal on September 17; existing public data and legacy publishing compatibility were preserved. Do not remove the Phase 1 operations or reset their counters.
...
Keep Vercel authentication on Preview, sandbox Stripe, Preview live Checkout disabled and `SUMMARY_AI_MONTHLY_BUDGET_USD=1`. Preserve summary service/account spending protection and the operations digest/cleanup guidance in [reconciliation](GENERATION_RECONCILIATION.md). Production migrations are applied, accounts are deployed, Google is public with verified branding, the live webhook delivers successfully and live Checkout is enabled with explicit owner authorization. Production retains its $25/month service-wide summary admission ceiling. Complete hosted paid upload/template/publication verification remains open.
