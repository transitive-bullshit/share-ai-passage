...
The accounts changes are local and not yet deployed. The owner approved the accounts feedback gate on September 15 and authorized Phase 2. Before deployment, apply the complete pending migration batch to an isolated preview database and verify hosted auth callbacks, fresh verification email delivery, deletion and recovery. Configure standard Better Auth, Google/GitHub and Resend values from the [setup checklist](THIRD_PARTY_SETUP.md). Account pages and APIs remain private/no-store or noindex as applicable; public readers and cards remain anonymously accessible.
...
The existing `APP_SECRET` is sufficient for Better Auth's normal fallback; a new `BETTER_AUTH_SECRET` is unnecessary. Leave `BETTER_AUTH_URL` unset to use Vercel's stable branch origin. Reuse the existing development Google/GitHub and Resend credentials in Preview, registering its exact OAuth callbacks alongside the local callbacks. Reuse the existing Stripe sandbox credentials and catalog for preview billing; the hosted webhook endpoint needs its own signing secret. Keep Stripe test/live settings distinct. The existing Google/GitHub, sandbox Stripe catalog and R2 values are now saved for Preview; existing Development Resend entries also include Preview. Shared APP_SECRET, OpenAI key and model settings remain in place. New image generation and live checkout are disabled. Hosted OAuth callbacks and the Stripe endpoint/signing secret await the actual deployment URL. Evidence: `work/phase2/hosted-readiness/shared-preview-env-evidence.json`. Configuration presence does not prove successful delivery or sign-in. Launch gates still apply.
...
The isolated HTTPS review hostname is not Internet reachable. Development events reach backend 3107 through the local Stripe listener, rather than an externally registered webhook endpoint. The current listener at `work/phase2/stripe/listen.mjs` forwards the following 20 billing event types; subscribe the hosted webhook to the same set:
...
Use separate public-card and private-asset buckets per environment. The examples use these S3 names; existing R2 configurations remain supported. A nonempty R2 value takes precedence over its S3 alias for each setting.
...
For development while a custom domain is pending, enable the **Public Development URL** only on the public-card bucket and copy its assigned `https://pub-<id>.r2.dev` origin into `S3_PUBLIC_URL`. This makes the entire bucket's objects public, not just a prefix. Keep every public access method disabled on the private bucket; its objects use short signed URLs. `r2.dev` is rate limited and intended for development. Before production launch, connect a custom domain to the public bucket, update the delivery origin, and disable its development URL if no longer needed. Do not CNAME a domain to `r2.dev`. [Public bucket access](https://developers.cloudflare.com/r2/buckets/public-buckets/)
...
**Development storage verified September 15:** the existing public `passage` bucket uses its `r2.dev` delivery origin, and the new `passage-private` bucket was created through the supplied US-jurisdiction S3 endpoint with public access disabled. The existing public GET/HEAD CORS rule was preserved. Private CORS was applied and read back for exactly `http://share-ai-passage.localhost:1355`, `https://share-ai-passage-accounts-afc0.local.share-ai-passage.com:8443`, and `https://www.share-ai-passage.com`, with the methods and headers above. Allowing the hosted origin does not constitute a production deployment.
...
An actual local Workflow queue/step roundtrip passed with a pre-cancelled synthetic operation and no provider request. This validates local routing and compilation, not hosted provider execution. Local `*:prod` commands explicitly use `.next-prod/workflow-data` and their own loopback origin; development uses its separate local history. On Vercel, verify Workflow/Queues availability, function duration for the 180-second provider deadline, environment identity, durable restart/replay, and actual event/queue/compute charges. Standalone/self-hosted replicas require a supported shared Workflow world; local filesystem history is not a multi-instance production queue.
...
**Gate B is open:** sandbox subscription/pack purchase and prorated upgrade, browser upload/template persistence, and synthetic R2 publication/recovery checks have passed. Remaining billing lifecycles, production storage delivery, image model qualification and supported input-cost bounds, and hosted operational measurements/alerts and recovery remain outstanding. The [measured economics report](research/PHASE2_MEASURED_ECONOMICS.md) distinguishes observed sample costs from workload assumptions. Keep public paid checkout and new image generation disabled until the complete paid flow, quality, economics and deployment checks pass.
