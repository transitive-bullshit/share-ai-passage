...
Local accounts review uses `http://share-ai-passage-accounts-afc0.localhost:1355`, with an exact `/api/auth/callback/github` callback registered on [Passage Development](https://github.com/settings/applications/3857667). The main checkout retains `http://share-ai-passage.localhost:1355` and its existing callback. Both callbacks have wildcard matching disabled. This worktree route maps to the isolated backend on port 3107; use its configured hostname for auth rather than the backend address. Google and hosted preview callbacks still need verification.
