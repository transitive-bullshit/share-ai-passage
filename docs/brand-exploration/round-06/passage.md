...
**Status:** Original Passage retained unchanged from the current-homepage extraction as a reference in round 06. This documents the existing name, typography, mark, copy, color system, and imagery. Two new Folio hybrids join original Folio and this original Passage identity in the comparison; this reference is not a new Passage treatment or an accepted final brand.
...
Folio and the two new hybrids use **Create a share**. Original Passage retains its existing **Go** form action and **Create a passage** navigation wording.

## Mark and typography

- **Mark:** Overlapping pages (`markStyle: outline`). The existing two outlined pages, unchanged from the homepage BrandMark component.
...
This is comparison content adapted in native rendering using Passage’s existing unthemed white/Inter card layout. It is **not a screenshot**, a new Passage style, or the live homepage’s unchanged example text. The comparison keeps content constant so the four options can be judged fairly.
...
See the [round README](README.md) for regeneration and the actual build/verification status. The extraction remains an unchanged reference alongside original Folio and two new Folio hybrids. Changes to the existing Passage system are outside this extraction request; canonical identity documents, final export sets, and product rollout wait for a complete decision.
