...
for (const d of data.directions) {
  for (const [key, weight] of [
    [d.bodyFont, 400],
    [d.headingFont, 500],
    ...(d.id === 'passage' ? [['passageSemibold', 600] as const] : [])
  ] as const) {
...
function palette(c: Canvas, d: Direction) {
  ...
  c.text('Identity comparison · Round 05', 80, 1260, 18, d.bodyFont, fg, {
    opacity: 0.55
  })
  c.text(
    d.id === 'passage'
...
const contact = new Canvas(
  2400,
  980,
  'Folio and Passage — two identities — Round 05'
)
contact.rect(0, 0, 2400, 980, '#E9E8E4')
posters.forEach((p, i) => contact.nested(p, 14 + i * 1192, 14, 0.733))
...
const overview = new Canvas(
  2400,
  690,
  'Folio and Passage — same conversation — Round 05'
)
overview.rect(0, 0, 2400, 690, '#E9E8E4')
cards.forEach((card, i) => {
  overview.text(
    data.directions[i]!.name,
    29 + i * 1192,
    42,
    24,
    'folioMedium',
  ...
  overview.nested(card, 14 + i * 1192, 65, 0.975)
})
  ...
  JSON.stringify(
    {
      ...
      poster: [1600, 1300],
      shareCard: [1200, 630],
      contactSheet: [2400, 980],
      sharecardsOverview: [2400, 690],
...
console.log(
  'Built Folio and Passage identity posters, share cards, and comparison overviews'
)
